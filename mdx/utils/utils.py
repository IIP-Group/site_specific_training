...
from .model_weights import compute_lr_multipliers
from sionna.utils import ebnodb2no, expand_to_rank

import h5py


def save_weights(system, model_path, save_format='h5'):
    """Save model weights.
...
def training_loop(model, label, filename, training_logdir, training_seed,
    ...
    # Optional: freeze selected weights by name (minimal-invasive finetuning control).
    # If `freeze_weight_name_patterns` is a list of substrings, any trainable weight
    # whose `.name` contains one of these substrings will be excluded from updates.
    #
    # Example (in config):
    #   freeze_weight_name_patterns = ["noise_multiplier_mat", "skip_multiplier"]
    freeze_weight_name_patterns = getattr(sys_parameters, "freeze_weight_name_patterns", None)
    if freeze_weight_name_patterns is not None:
        assert isinstance(freeze_weight_name_patterns, (list, tuple)), \
            "freeze_weight_name_patterns must be a list of substrings"
        freeze_weight_name_patterns = [str(p) for p in freeze_weight_name_patterns]
    ...
    def _compile_step(batch_size, min_snr_db, max_snr_db, double_readout,
        ...
        for _ in tf.range(step, dtype=tf.int64):
            ...
            with tf.GradientTape() as tape:
                if sys_parameters.system=='mdx':
                    loss_data, loss_chest, bce_rel, loss_chest_all, loss_tilde, loss_tilde_all = model(batch_size, snr_db, num_tx,
            ...
            # Optionally exclude some trainable weights from updates (freeze-by-name)
            if freeze_weight_name_patterns:
                train_vars = []
                train_lr_muls = [] if lr_muls is not None else None
                for i, w in enumerate(model.trainable_weights):
                    if any(p in w.name for p in freeze_weight_name_patterns):
                        continue
                    train_vars.append(w)
                    if train_lr_muls is not None:
                        train_lr_muls.append(lr_muls[i])
            else:
                train_vars = model.trainable_weights
                train_lr_muls = lr_muls

            grads = tape.gradient(loss, train_vars)
            if train_lr_muls is None:
                optimizer.apply_gradients(zip(grads, train_vars))
            else:
                # Scale the gradients according to the multipliers
                scaled_grads = [g * m if g is not None else None for g, m in zip(grads, train_lr_muls)]
                optimizer.apply_gradients(zip(scaled_grads, train_vars))
    ...
    def _training_step(global_iter, num_iterations, batch_size,
        ...
        for _ in tf.range(int(num_iterations/step), dtype=tf.int64):
            ...
            try:
                if sys_parameters.system=='mdx':
                    loss_data, loss_chest, loss, bce_rel, loss_chest_all = _compile_step(batch_size,
            ...
            # if cgnn_step>0 and global_iter-git >= cgnn_step:
                
            #     num_it +=1
            #     if num_it>sys_parameters.num_nrx_iter:
            #         num_it = 1
            #     model._receiver._neural_rx.num_it = num_it
            #     git = global_iter
            ...
            if sys_parameters.system=='mdx':
                if isinstance(bce_rel, list):
                    ...
                    # for i, value in enumerate(loss_tilde_all):
                    #     tf.summary.scalar(f"CHE_tilde {i}", value, step=global_iter)
        ...
        return global_iter, loss, loss_data, loss_chest, num_it, git
    ...
    with summary_writer.as_default():
        ...
        for i, num_iterations in enumerate(training_schedule["num_iter"]):
            ...
            for _ in range(num_iter_global):
                ...
                global_iter, loss, loss_data, loss_chest, num_it, git = \
                ...
                for mcs_arr_idx in mcs_arr_training_idx:
                    ...
                    if sys_parameters.system=='mdx':             
                        if isinstance(bce_rel, list):
                            ...
                            # for i, value in enumerate(loss_tilde_allv):
                            #     tf.summary.scalar(f"Eval CHE_tilde {i} mcs:" + str(mcs_arr_idx), value, step=global_iter)                
                tf.summary.scalar(f"Loss", loss_data, step=global_iter)
