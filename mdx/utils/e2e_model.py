# Copy of the original file from MAbdollahpour/MDX with an additional 
# datalake path to allow for real world data training using the DataLakeChannel class in channel_models.py.
...
from sionna.utils import BinarySource, ebnodb2no, expand_to_rank, log10, flatten_dims, split_dim
from .baseline_rx import BaselineReceiver
from .neural_rx import NeuralPUSCHReceiver
from .md_rx import MDNeuralPUSCHReceiver
...
class E2E_Model(Model):
    ...
    def __init__(self, sys_parameters, training=False, return_tb_status=False,
        ...
        if self._sys_parameters.system == 'baseline_perf_csi_kbest':
            ...
            self._receiver = BaselineReceiver(
                                ...
                                return_tb_status=return_tb_status,
                                mcs_arr_eval_idx=mcs_arr_eval_idx)
        elif self._sys_parameters.system == 'baseline_lslin_kbest':
            self._sys_name = f"Baseline - LS/lin+K-Best"
            self._receiver = BaselineReceiver(
                                self._sys_parameters,
                                return_tb_status=return_tb_status,
                                mcs_arr_eval_idx=mcs_arr_eval_idx)
            ...
            self._receiver = NeuralPUSCHReceiver(
                                    self._sys_parameters,
                                    training)
        elif self._sys_parameters.system == "mdx":
            self._sys_name = "Model Driven Receiver"
            self._receiver = MDNeuralPUSCHReceiver(
                                    self._sys_parameters,
                                    training)
        else:
    ...
    def call(self, batch_size, ebno_db, num_tx=None, output_nrx_h_hat=False,  # ---------------------------  Call ---------------------------
        ...
        if self._sys_parameters.channel_type != "Datalake":
            ...
            # mcs_ue_mask: [batch_size, max_num_tx, depth], depth being num supported MCSs
            # _mcs_ue_mask:[batch_size, max_num_tx, num_selected_mcs(1), 1, 1]
            # transmitters is a list of PUSCHTransmitters, one for each MCS
            # self._transmitters[mcs_arr_eval[0]](b[0]): [batch_size, max_num_tx, tx_ant, num_ofdm_syms, fft_size]
            # x: [batch_size, max_num_tx, tx_ant, num_ofdm_syms, fft_size]
            _mcs_ue_mask = tf.cast(expand_to_rank(
                                        tf.gather(mcs_ue_mask,
                                                indices=mcs_arr_eval[0],
                                                axis=2), 5, axis=-1),
                                                dtype=tf.complex64)
            x = _mcs_ue_mask * self._transmitters[mcs_arr_eval[0]](b[0]) # mul by zeros all tx with MACs != mcs_arr_eval[0]
            ...
            for idx in range(1, len(mcs_arr_eval)):
                _mcs_ue_mask = tf.cast(expand_to_rank(
                                            tf.gather(mcs_ue_mask,
                                                    ...
                                                    axis=2), 5, axis=-1),
                                                    dtype=tf.complex64)
                x = x + _mcs_ue_mask * self._transmitters[mcs_arr_eval[idx]](b[idx])# mul by zeros all tx with MACs != mcs_arr_eval[idx]
        ...
        else: # Datalake channel
            # Validate DataLake configuration
            assert self._sys_parameters.datalake_channel is not None, \
                "For Datalake channel, datalake_channel must be specified in sys_parameters"
            
            # Get data from DataLake. The channel returns an active-user mask
            # aligned with how the (fixed-layer) capture is laid out, so we
            # override the randomly-sampled active_dmrs for the Datalake path.
            dl_ch = self._sys_parameters.datalake_channel
            y, h, b, h_ls, no, active_dmrs = dl_ch.get_batch(batch_size)
            no = 0.1 # dummy no for now, Future work: modify receiver to make no optional when using datalake channel.
            h = None
            # `no` comes from TFRecord (per-sample). Older TFRecords that don't
            # contain it are automatically defaulted inside `DataLakeChannel`.
            x = None
        ...
        if self._sys_parameters.system in ('baseline_lmmse_kbest',
            ...
            if self._training:
                losses = self._receiver([y, active_dmrs, b, h, mcs_ue_mask],
                                        mcs_arr_eval,
                                        h_ls=h_ls if self._sys_parameters.channel_type=="Datalake" else None)
                return losses
            else:
            ...
            # in training mode, only the losses are required
            if self._training:
                losses = self._receiver([y, active_dmrs, b, h, mcs_ue_mask, x, no, batch_size],
                                        mcs_arr_eval,
                                        h_ls=h_ls if self._sys_parameters.channel_type=="Datalake" else None)
