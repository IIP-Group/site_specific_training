# Modified from the original by adding a DataLake Channel path for real-world training and testing on a fixed data scrambling configuration.
...
class E2E_Model(Model):
    ...
    def __init__(self, sys_parameters, training=False, return_tb_status=False,
        ...
        elif self._sys_parameters.system == "lsnn_duidd":
            self._sys_name = "DUIDD - LS/nn+DUIDD"
            self._receiver = BaselineReceiver(
                                self._sys_parameters,
                                return_tb_status=return_tb_status,
                                mcs_arr_eval_idx=mcs_arr_eval_idx,
        ...
        elif self._sys_parameters.system == "lslin_duidd":
            self._sys_name = "DUIDD - LS/lin+DUIDD"
            self._receiver = BaselineReceiver(
                                self._sys_parameters,
                                return_tb_status=return_tb_status,
                                mcs_arr_eval_idx=mcs_arr_eval_idx,
        ...
        elif self._sys_parameters.system == "lmmse_duidd":
            self._sys_name = "DUIDD - LMMSE+DUIDD"
            self._receiver = BaselineReceiver(
                                self._sys_parameters,
                                return_tb_status=return_tb_status,
                                mcs_arr_eval_idx=mcs_arr_eval_idx,
        ...
        elif self._sys_parameters.system == "perf_csi_duidd":
            self._sys_name = "DUIDD - Perf. CSI + DUIDD"
            self._receiver = BaselineReceiver(
                                self._sys_parameters,
                                return_tb_status=return_tb_status,
                                mcs_arr_eval_idx=mcs_arr_eval_idx,
    ...
    def call(self, batch_size, ebno_db, num_tx=None, output_nrx_h_hat=False,
        ...
        if self._sys_parameters.channel_type != "Datalake":
            ...
            assert self._sys_parameters.datalake_channel is not None, \
                "For Datalake channel, datalake_channel must be set in Parameters"
            y, b, no, active_dmrs, bits_info = self._sys_parameters.datalake_channel.get_batch(
                batch_size, num_tx, return_b_info=True)
            h = None
        ...
        _duidd_systems = ("lsnn_duidd", "lslin_duidd", "lmmse_duidd", "perf_csi_duidd")
        if self._sys_parameters.system in ('baseline_lmmse_kbest',
                                           ...
                                           "lmmse_duidd"):
            if (self._training
                    and self._sys_parameters.channel_type == "Datalake"
                    and self._sys_parameters.system in _duidd_systems):
                rx_return_arguments = self._receiver(
                    [y, no], b_info=bits_info, active_dmrs=active_dmrs)
            else:
                ...
                rx_return_arguments = self._mask_active_dmrs(
            ...
            # b only holds bits corresponding to MCS indices specified
            # in mcs_arr_eval --> evaluation for one MCS only --> b[0]
            rx_return_arguments = self._mask_active_dmrs(b[0], b_hat, num_tx,
        ...
        if self._training:
            return self.loss_fun(rx_return_arguments)
        else:
            return rx_return_arguments
