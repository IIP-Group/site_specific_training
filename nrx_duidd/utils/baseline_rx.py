...
class BaselineReceiver(Layer):
    ...
    def __init__(self,
        ...
        elif sys_parameters.system in ('baseline_lsnn_lmmse', 'lsnn_duidd'):
            pc = sys_parameters.pusch_configs[mcs_arr_eval_idx][0]
            self._est = PUSCHLSChannelEstimator(
                resource_grid=sys_parameters.transmitters[mcs_arr_eval_idx]._resource_grid,
                dmrs_length=pc.dmrs.length,
        ...
        elif sys_parameters.system in ('baseline_lslin_lmmse', 'lslin_duidd'):
            pc = sys_parameters.pusch_configs[mcs_arr_eval_idx][0]
            self._est = PUSCHLSChannelEstimator(
                resource_grid=sys_parameters.transmitters[mcs_arr_eval_idx]._resource_grid,
                dmrs_length=pc.dmrs.length,
        ...
        if sys_parameters.system in ('perf_csi_duidd', 'nn_duidd', 'lslin_duidd', 'lmmse_duidd'):
            self._receiver = DUIDDPUSCHReceiver(
    ...
    def call(self, inputs, b_info=None, active_dmrs=None, bit_grid=None):
        # Datalake training: forward labels into the inner DUIDD receiver
        if ((b_info is not None or bit_grid is not None)
                and self._training
                and self._sys_parameters.channel_type == "Datalake"):
            y, no = inputs
            return self._receiver(
                [y, no], b_info=b_info, active_dmrs=active_dmrs,
                bit_grid=bit_grid)
