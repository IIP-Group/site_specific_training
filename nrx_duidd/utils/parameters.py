...
class Parameters:
    ...
    def __init__(self, config_name, system="dummy", training=False, verbose=False, compute_cov=False, num_tx_eval=None):
        ...
        # DoubleTDL for evaluation
            ...
            self.channel = None
            num_subcarriers = self.n_size_bwp * 12
            num_ofdm_symbols = self.symbol_allocation[1]
            resource_grid_shape = (num_subcarriers, num_ofdm_symbols)
            datalake_no = getattr(self, "datalake_no", None)
        ...
        if self.system in ("baseline_lmmse_kbest", "baseline_lmmse_lmmse", "lmmse_duidd"):
