...
def datalake_info_bce(labels, logits, active_dmrs):
    """BCE on TB info bits vs soft output-decoder LLRs (matches eval BLER/BER)."""
    labels = tf.cast(labels, tf.float32)
    num_tx = tf.shape(labels)[1]
    loss = tf.nn.sigmoid_cross_entropy_with_logits(
        labels=labels, logits=logits)
    loss = tf.reduce_mean(loss, axis=-1)
    active = tf.cast(active_dmrs[:, :num_tx], tf.float32)
    loss = loss * active
    return tf.math.divide_no_nan(
        tf.reduce_sum(loss), tf.reduce_sum(active))


def _bce_logsumexp_normalized(bce, axis=-1):
    """Paper / BLER_Training normalized LSE: ``log(sum(exp(l_i)) - D + 1)``.

    Mathematically ``>= 0`` for BCE ``l_i >= 0``, but the float32 shifted form
    used in BLER_Training cancels for large D (e.g. ~1.6e5 TB bits) and yields
    a non-positive log argument → NaN loss → NaN IDD weights → LMMSE
    ``MatrixInverse`` failures on the next step. Compute in float64 and clamp.
    """
    x = tf.cast(bce, tf.float64)
    x_max = tf.reduce_max(x, axis=axis, keepdims=True)
    x_max_ = tf.squeeze(x_max, axis=axis)
    n_bits = tf.cast(tf.shape(x)[axis], tf.float64)
    # log(sum(exp(x)) - (n-1)) = x_max + log(sum(exp(x-x_max)) - (n-1)exp(-x_max))
    inner = (tf.reduce_sum(tf.exp(x - x_max), axis=axis)
             - (n_bits - 1.) * tf.exp(-x_max_))
    # Theory: inner * exp(x_max) = sum(exp(x))-(n-1) >= 1; clamp float noise.
    inner = tf.maximum(inner, 1e-12)
    return tf.cast(x_max_ + tf.math.log(inner), bce.dtype)


def datalake_info_bce_lse(labels, logits, active_dmrs, normalized=False):
    """LogSumExp of per-bit BCE over TB info bits (BLER surrogate).

    Matches IIP-Group/BLER_Training ``computeLoss`` with ``BCE_LogSumExp`` /
    ``BCE_LogSumExp_normalized``: per-bit BCE first, then aggregate over the
    bit axis with log-sum-exp (soft-max), then mean over active DMRS ports.
    """
    labels = tf.cast(labels, tf.float32)
    num_tx = tf.shape(labels)[1]
    bce = tf.nn.sigmoid_cross_entropy_with_logits(
        labels=labels, logits=logits)
    if normalized:
        loss = _bce_logsumexp_normalized(bce, axis=-1)
    else:
        loss = tf.math.reduce_logsumexp(bce, axis=-1)
    active = tf.cast(active_dmrs[:, :num_tx], tf.float32)
    loss = loss * active
    return tf.math.divide_no_nan(
        tf.reduce_sum(loss), tf.reduce_sum(active))
...
def datalake_masked_bce(labels, logits, active_dmrs):
    """Masked BCE for datalake bit-grid labels vs soft LLRs.

    Same recipe as ``neural_rx`` ``CGNNOFDM`` Datalake training:
    ignore sentinel ``-1`` (DMRS / no-data), sanitize labels to 0 for BCE,
    normalize by the number of active data bits, then zero inactive DMRS ports.
    """
    batch_size = tf.shape(labels)[0]
    num_tx = tf.shape(labels)[1]
    # Flatten RE/bit dims so masking matches neural_rx (B, num_tx, -1)
    labels_flat = tf.reshape(tf.cast(labels, tf.float32),
                             (batch_size, num_tx, -1))
    logits_flat = tf.reshape(logits, (batch_size, num_tx, -1))

    # Build mask once before sanitizing -1 -> 0
    mask = tf.cast(labels_flat != -1., tf.float32)
    labels_safe = tf.where(labels_flat == -1., 0., labels_flat)

    loss = tf.nn.sigmoid_cross_entropy_with_logits(
        labels=labels_safe, logits=logits_flat)
    loss = loss * mask
    # Fully masked users (all -1) -> 0 via divide_no_nan, not NaN
    loss = tf.math.divide_no_nan(
        tf.reduce_sum(loss, axis=-1),
        tf.reduce_sum(mask, axis=-1))

    active = expand_to_rank(active_dmrs[:, :num_tx], tf.rank(loss), axis=-1)
    loss = loss * active
    return tf.reduce_mean(loss)


def set_loss_function(model, loss_fun_name: str, datalake=False):

    ################
    ##### Loss #####
    ################

    print("Loss function: " + loss_fun_name
          + (" (datalake)" if datalake else ""))
    # Per-bit BCE is always the base; aggregation differs by loss_fun_name.
    model._loss_fun_ = tf.nn.sigmoid_cross_entropy_with_logits

    if loss_fun_name == "bce":
        def _aggregate_bce(bce):
            return tf.reduce_mean(bce)

        def _datalake_loss(labels, llrs, active_dmrs):
            return datalake_info_bce(labels, llrs, active_dmrs)

    elif loss_fun_name in ("bce_lse", "bce_logsumexp"):
        # BLER_Training: cost = reduce_logsumexp(per_bit_bce, axis=-1)
        def _aggregate_bce(bce):
            return tf.reduce_mean(tf.math.reduce_logsumexp(bce, axis=-1))

        def _datalake_loss(labels, llrs, active_dmrs):
            return datalake_info_bce_lse(
                labels, llrs, active_dmrs, normalized=False)

    elif loss_fun_name in ("bce_lse_normalized", "bce_logsumexp_normalized"):
        # BLER_Training / paper: log(sum(exp(bce)) - D + 1)
        def _aggregate_bce(bce):
            return tf.reduce_mean(_bce_logsumexp_normalized(bce, axis=-1))

        def _datalake_loss(labels, llrs, active_dmrs):
            return datalake_info_bce_lse(
                labels, llrs, active_dmrs, normalized=True)

    else:
        raise NotImplementedError("Loss not implemented: " + loss_fun_name)

    if datalake:
        def loss_fun(inputs):
            # Soft info-bit LLRs from output decoder vs TFRecord b_info (pduData)
            labels, llrs, active_dmrs = inputs
            loss_data = _datalake_loss(labels, llrs, active_dmrs)
            loss_chest = tf.constant(0., dtype=tf.float32)
        ...
        def loss_fun(inputs):
            if len(inputs) == 3:
                ...
                b, llr = inputs
            loss_data = _aggregate_bce(model._loss_fun_(b, llr))
            loss_chest = 0
            return loss_data, loss_chest

    model.loss_fun = loss_fun
...
def training_loop(model, label, filename, training_logdir, training_seed,
                  ...
                  weight_saving_schedule=None, duidd=False, xla=False):
    # pylint: disable=line-too-long
    r"""
    ...
    def _compile_step(batch_size, min_snr_db, max_snr_db, double_readout,
                      weighting_double_readout, apply_multiloss, train_tx, loss_fun_name=""):
        ...
        if duidd:
            set_loss_function(
                model, loss_fun_name,
                datalake=(sys_parameters.channel_type == "Datalake"))
        ...
        for _ in tf.range(int(num_iterations/100), dtype=tf.int64):
            ...
            if duidd:
                for weights in model.weights:
                    ...
                    if len(np.shape(weights_np)) > 0:
                        for i in range(len(weights_np)):
                            if len(np.shape(weights[i]))>0:
                                for j in range(len(weights[i])):
                                    tf.summary.scalar(f"Weight" + weights.name + " / idx=" + str(i)+","+str(j), weights[i, j], step=global_iter)
                            else:
                                tf.summary.scalar(f"Weight" + weights.name + " / idx=" + str(i), weights[i], step=global_iter)
                    else:
                        tf.summary.scalar(f"Weight" + weights.name, weights_np, step=global_iter)
    ...
    with summary_writer.as_default():
        ...
        for i, num_iterations in enumerate(training_schedule["num_iter"]):
            ...
            if duidd:
                loss_fun_name = training_schedule["loss_fun_name"][i]
            else:
                loss_fun_name = ""
