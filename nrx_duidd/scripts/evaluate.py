...
parser.add_argument("-duidd", help="NRX is DUIDD receiver",
                    action="store_true", default=False)
...
for num_tx_eval in num_tx_evals:
    ...
    if not eval_nrx_only or (duidd and sys_parameters.chest == "lmmse"):
    ...
    for mcs_arr_eval_idx in mcs_arr_eval_idxs:
        ...
        # Neural receiver
        #
        sn.config.xla_compat = not duidd
        sys_parameters = Parameters(config_name,
                                    training=False,
                                    num_tx_eval=num_tx_eval,
                                    system='nrx' if not duidd else sys_parameters.chest+'_duidd')
        ...
        if duidd and sys_parameters.chest == 'lmmse':
            e2e_nn(sys_parameters.batch_size_eval_small,
                   tf.ones([sys_parameters.batch_size_eval_small], dtype=tf.float32))
        else:
            e2e_nn(1, 1.)
        ...
        if exists(filename):
            ...
            print("### Weights not found! Running with standard values.")
        ...
        ber, bler = sim_ber(e2e_nn,
                            graph_mode="xla" if not duidd else "graph",
                            ebno_dbs=ebno_db,
                            max_mc_iter=max_mc_iter,
                            num_target_block_errors=num_target_block_errors,
                            batch_size=batch_size if (not duidd or sys_parameters.chest != "chest") else sys_parameters.batch_size_eval_small,
        ...
        if not eval_nrx_only:
            ...
            ber, bler = sim_ber(e2e_baseline,
                            ...
                            num_target_block_errors=num_target_block_errors,
                            target_bler=target_bler,
                            batch_size=batch_size_small,  # LMMSE chest is expensive
                            early_stop=True,
                            forward_keyboard_interrupt=True)
            BERs[e2e_baseline._sys_name, num_tx_eval, mcs_arr_eval_idx] = ber
            BLERs[e2e_baseline._sys_name, num_tx_eval, mcs_arr_eval_idx] = bler
