...
parser.add_argument("-iter", help="Number of iterations suffix for loading weights", type=str, default=None)
...
# iter = "_100_iter"
iter = "" if args.iter is None else f"_{args.iter}_iter"
# iter = ""
# load weights
print("Loading pre-trained weights.")
load_weights(neural_rx, f"../weights/{sys_parameters.label}_weights{iter}")
...
neural_rx.save(f"../onnx_models/{sys_parameters.label}_tf{iter}")
...
onnx.save(onnx_model,f"../onnx_models/{sys_parameters.label}{iter}.onnx")
...
trt_command = f'trtexec --fp16 '\
    f'--onnx=../onnx_models/{sys_parameters.label}{iter}.onnx '\
    f'--saveEngine=../onnx_models/{sys_parameters.label}{iter}.plan '#\
