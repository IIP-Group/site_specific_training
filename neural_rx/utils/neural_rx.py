# Copy of the original file from NVabs/neural_rx with an added loss calculation 
...
class CGNNOFDM(Model):
    ...
    def call(self, inputs, mcs_arr_eval, mcs_ue_mask_eval=None):
        ...
        for llrs_, h_hat_ in zip(llrs_, h_hats_):
            ...
            is_datalake = getattr(self._sys_parameters, "channel_type", None) == "Datalake"
            for idx in indices:
                ...
                if self._training and is_datalake:
                    (B, R) = llrs_[idx].shape[0:2]
                    llrs_[idx] = tf.reshape(llrs_[idx], (B, R, -1))
                    
                    # bits[idx] = tf.cast(bits[idx], tf.float32)
                    # keep_indices, _ = tf.unique(tf.where(bits_flat != -1)[:, 2])
                    # bits[idx]= tf.gather(bits_flat, keep_indices, axis=2)
                    # llrs_[idx] = tf.gather(llrs_flat, keep_indices, axis=2)
                    # [batch_size, num_tx, num_data_symbols*num_bits_per_symbol]

                # Only apply TEST modifications when not training
        ...
        # Datalake: build the DMRS/no-data loss mask ONCE, before the labels
        # are sanitized, and keep one mask per evaluated MCS.
        # Doing this inside the per-iteration loop above is a bug: it overwrites
        # bits[idx] (turning the -1 sentinels into 0), so on the 2nd and later
        # iterations the mask `bits_flat != -1` collapses to all-ones. The loss
        # loop below then runs with an all-ones mask whenever num_it >= 2,
        # leaking pilot/no-data REs into the BCE loss and mis-normalizing it.
        dmrs_loss_mask = None
        if self._training and is_datalake:
            dmrs_loss_mask = []
        ...
        if self._training:
            ...
            for llrs_ in llrs:
                for idx in range(len(indices)):
                    is_datalake = getattr(self._sys_parameters, "channel_type", None) == "Datalake"
                    if is_datalake:
                        # use this so that it doesn't collapse the last dimension
                        # masking the loss corresponding to DMRS is more straightforward this way
...
class NeuralPUSCHReceiver(Layer):
    ...
    def estimate_channel(self, y, num_tx):
        ...
        if self._sys_parameters.initial_chest == 'ls':
            ...
            h_hat, _ = self._ls_est([y, 1e-1]) # inputs are provided separately in Sionna 1.0 (as opposed to [y, N0])
    ...
    def preprocess_channel_ground_truth(self, h):
        ...
        # [batch size, num_tx, num_ofdm_symbols, num_effective_subcarriers,
        #   num_rx_ant]
        # h = tf.squeeze(h, axis=-1) # TEST TEST TEST TEST TEST
        h = tf.squeeze(tf.matmul(h, w), axis=-1) #TEST TEST TEST TEST TEST
    ...
    def call(self, inputs, mcs_arr_eval=[0], mcs_ue_mask_eval=None, h_ls=None):
        ...
        if self._training:
            ...
            # return llr # TEST TEST TEST TEST TEST
