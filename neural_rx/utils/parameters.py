...
from .channel_models import DoubleTDLChannel, DatasetChannel, DataLakeChannel
...
class Parameters:
    ...
    def __init__(self, config_name, system="dummy", training=False, verbose=False, compute_cov=False, num_tx_eval=None):
        ...
        # figs = pilot_pattern.show(tx_ind=[0, 1])
        # [fig.savefig(f"pilot_pattern_tx{tx}.png", dpi=200, bbox_inches="tight") for tx, fig in enumerate(figs)]
        # plt.close("all")
        ...
        # DoubleTDL for evaluation
            ...
            # For mixed-layer training set datalake_tf_fn to a list
            # [fn_1tx, fn_2tx] and min_num_tx < max_num_tx. When
            # min_num_tx == max_num_tx (e.g. during evaluation, or single-layer
            # training) DataLakeChannel falls back to the single-dataset path.
            self.datalake_channel = DataLakeChannel(
                tf_fn=self.datalake_tf_fn,
                n_size_bwp = self.n_size_bwp,
                max_num_tx=self.max_num_tx,
                min_num_tx=self.min_num_tx,
