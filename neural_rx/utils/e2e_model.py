# Copy of the original file from NVabs/neural_rx with an additional 
...
from sionna.utils import ebnodb2no, expand_to_rank, log10, BinarySource
...
class E2E_Model(Model):
    ...
    def map_bits_to_resource_grid(self, tb_input, transmitter):
        ...
        # precoder = transmitter._precoder

        mod_order = 4 # for 16QAM
        ...
        tb_encoded_bits = tb_encoder(tb_input)
        ...
        # symbol_bits = tf.expand_dims(symbol_bits, axis=1)
        # symbol_bits = tf.reshape(symbol_bits, (1, 1, -1, mod_order))
        # weights = tf.constant([8., 4., 2., 1.]) # for 16QAM convert 4 bits to int 0-15
        weights = tf.pow(2, tf.range(mod_order - 1, -1, -1, dtype=symbol_bits.dtype)) # for 16QAM convert 4 bits to int 0-15
    ...
    def call(self, batch_size, ebno_db, num_tx=None, output_nrx_h_hat=False,
        ...
        if num_tx is None:
            num_tx = self._sys_parameters.max_num_tx # TEST TEST TEST TEST TEST
            # num_tx = 1 # TEST TEST TEST TEST TEST
        ...
        if self._sys_parameters.channel_type != "Datalake":
            ...
            # if self._training:
            #     bit_grid = self.map_bits_to_resource_grid(b[0], self._transmitters[mcs_arr_eval[0]])
            #     b[0] = bit_grid
            #     # pass
            ...
            y, b, h_ls, active_dmrs = dl_ch.get_batch(batch_size, num_tx)
            h = None
        ###################################
        # Receiver
        ###################################
        ...
        if self._sys_parameters.system in ('baseline_lmmse_kbest',
        ...
        elif self._sys_parameters.system in ('baseline_perf_csi_kbest', 'baseline_perf_csi_lmmse'):
            ...
            if self._training:
                ...
                # llr = self._receiver((y, active_dmrs),
                #                         mcs_arr_eval,
